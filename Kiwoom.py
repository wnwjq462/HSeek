...
class Kiwoom(QAxWidget):
    ...
    def _opt10081(self, rqname, trcode):
        ...
        data_list = self.dynamicCall("GetCommDataEx(QString, QString)",trcode, "주식일봉차트조회")
        '''
        ...
        for j in range(len(data_list)) :
            ...
            low = data_list[j][7]
            close = data_list[j][1]
            volume = data_list[j][2]

            if len(self.ohlcv['날짜']) != 0 :
                if close == '0' or close == 0 :
                    price_rate = 0
                else :
                    price_rate = self.ohlcv['종가'][-1] / float(close)
                    price_rate = (price_rate - 1 ) * 100
                    price_rate = round(price_rate, 2)

                self.ohlcv['등락율'].append(price_rate)

                if volume == '0' or volume == 0:
                    volume_rate = 0
                else :
                    volume_rate = (self.ohlcv['거래량'][-1] / float(volume)) * 100
                    volume_rate = round(volume_rate,2)

                self.ohlcv['거래대비'].append(volume_rate)

            self.ohlcv['날짜'].append(date)
            self.ohlcv['시가'].append(int(open))
            self.ohlcv['고가'].append(int(high))
            self.ohlcv['저가'].append(int(low))
            self.ohlcv['종가'].append(int(close))
            self.ohlcv['거래량'].append(int(volume))
...
if __name__ == "__main__":
    ...
    kiwoom = Kiwoom()
    kiwoom.comm_connect()
    kiwoom.ohlcv = {'날짜': [], '시가': [], '고가': [], '저가': [], '종가': [], '거래량': [], '등락율' : [], '거래대비' : []}
    '''
    # opt10081 TR 요청
    ...
    print(kiwoom.ohlcv)
    df = pd.DataFrame(kiwoom.ohlcv, columns=['시가', '고가', '저가', '종가', '거래량', '등락율', '거래대비'], index=kiwoom.ohlcv['날짜'])
